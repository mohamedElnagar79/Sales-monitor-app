  ...
  try {
    ...
    if (isUpdate == true) {
      writeStream = fs.createWriteStream(
        `./public/invoices/updates/invoice_${invoiceData.id}.pdf`
      );
    }
    writeStream = fs.createWriteStream(
    ...
    const tableWidth =
      productNameWidth + quantityWidth + priceWidth + totalWidth;
    ...
    newInvoiceItems.forEach(async (item) => {
      ...
      if (isUpdate) {
        ...
        item.productName = product.dataValues.name;
      ...
      doc.text(
        isNaN(item.piecePrice)
          ? "0.00"
          : parseFloat(item.piecePrice).toFixed(2),
        50 + productNameWidth + quantityWidth + paddingLeft,
        y + priceYPosition,
        { width: priceWidth, align: "left" }
      );
